...
IGNORE_LABEL_ID = -100
DEFAULT_EPSILON = 1e-8 # Epsilon for diagonal Hessian approximation / RMSProp
...
class TinyRecursiveReasoningModel_ACTV1Config_DyT(BaseModel):
    ...
    ebt_hessian_eps: float = DEFAULT_EPSILON # Epsilon for stabilizing Hessian division / RMSProp
    ebt_beta2: float = 0.99 # Beta2 for RMSProp running average (New)
...
class TinyRecursiveReasoningModel_ACTV1_Inner_DyT_EBT2(nn.Module):
    """The core inner loop model combining TRM-DyT with 2nd Order EBT guidance (RMSProp Preconditioner)."""
    def __init__(self, config: TinyRecursiveReasoningModel_ACTV1Config_DyT) -> None:
        ...
        # Buffer to store the running average of squared gradients for RMSProp preconditioning
        # Initialized on first forward pass or explicitly to match device/dtype of z_L
        self.register_buffer('exp_avg_sq', None, persistent=False)
        self.beta2 = self.config.ebt_beta2 # Beta for RMSProp (e.g., 0.99 or 0.999)
        # --- End RMSProp State ---
    ...
    def reset_carry(self, reset_flag: torch.Tensor, carry: TinyRecursiveReasoningModel_ACTV1InnerCarry_DyT):
        ...
        if self.exp_avg_sq.shape[1] != state_shape[1]:
             # Re-init buffer if shape is wrong (e.g. first batch)
    ...
    def forward(self, carry: TinyRecursiveReasoningModel_ACTV1InnerCarry_DyT, batch: Dict[str, torch.Tensor]) -> Tuple[TinyRecursiveReasoningModel_ACTV1InnerCarry_DyT, torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # Initialize exp_avg_sq buffer on first forward pass if needed
        ...
        # Slice the RMSProp buffer for this batch
        ...
        for _L_step in range(self.config.L_cycles):
            ...
            # Update running average of squared gradients (RMSProp style)
            current_grad_sq = energy_grad.detach().square()
            # In-place update of the SLICED buffer
        ...
        if pad_needed > 0:
            ...
            # Write the updated sliced buffer back to the full buffer
            self.exp_avg_sq[:, :current_total_seq_len, :] = exp_avg_sq_sliced
        else:
            z_H_padded = z_H
            z_L_padded = z_L
            self.exp_avg_sq = exp_avg_sq_sliced # It's not a slice, it's the whole buffer
...
class TinyRecursiveReasoningModel_ACTV1_DyT_EBT2(nn.Module):
    """ACT wrapper managing the outer loop (steps, halting) for the EBT2-enhanced TRM."""
    def __init__(self, config_dict: dict):
        ...
        self.inner = TinyRecursiveReasoningModel_ACTV1_Inner_DyT_EBT2(self.config)
